...
Next, the table shows the total activity counts via aggregation across minutes for each day. . Are any trends apparent? We can see that the subjective is mostly more engaged with physical activity during weekdays than druing weekends. The subject typically alternates the level of physical activity on alternate days. The total_activity for Saturday week 4 and 5 returns as 1440 results from summing up of every account count of value "1" during the whole day. The possible reason is that the subjective didn't wear/use accccelerometer on Satudays in week 4 and 4 for personal reasons.  
...
Below is a single-panel plot that shows the 24-hour activity time courses for each day and use color to indicate day of the week. !!!!!!!Describe in words any patterns or conclusions you can make based on this graph. The baseline of physical activity across one week is below 2500 every minute. The subject engaged with more physical activity early in the day during weekdays and Saturday while later in the day on Sunday. The possible reason is that the subject exercises in the morning in the first 6 days but exercises in the afternoon on Sunday. The subject is more active in approximately fixed periods of time during 24 hours throughout 5 weeks.
...
Below is two-panel plot showing the average max temperature in January and in July in each station across years. The general pattern average maximumd daily temperature in each station is similar across years 1981-2010. We see a fluctuations between -10C and 10 C in January and 20C to ~32C where the line graph is darkest due to overlying. The notible outlier in January is in 1982 where the average max daily temperature is much below -10C. The outlier in July is in 1988 where the average max temperature is below 15C.
```{r}
ny_noaa %>% 
